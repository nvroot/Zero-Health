...
const Chatbot = ({ user }) => {
  ...
  useEffect(() => {
    const loadChatHistory = async () => {
      ...
      try {
        ...
        const response = await fetch('http://nc5.netbird.selfhosted:6002/api/chatbot/history', {
          headers: {
  ...
  const sendMessage = async () => {
    ...
    try {
      const userData = localStorage.getItem('user');
      const token = userData ? JSON.parse(userData).token : null;
      const response = await fetch('http://nc5.netbird.selfhosted:6002/api/chatbot/chat', {
        method: 'POST',
        headers: {
